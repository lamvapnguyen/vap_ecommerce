        ...
        public string SECTION_NAME
        {
            get
            {
                using (DBHelpDataContext data = new DBHelpDataContext())
                {
                    try
                    {
                        M_STAFF staff = data.M_STAFFs.FirstOrDefault(c => c.STAFF_ID == this.STAFF_ID);
        ...
        public string EVALUATION_INDIVIDUAL
        {
            get
            {
                using (DBHelpDataContext data = new DBHelpDataContext())
                {
                    try
                    {
                        return Common.ParseString(data.T_STAFF_EMPLOYMENT_HISTORies.FirstOrDefault(c => c.STAFF_ID == this.STAFF_ID).EVALUATION_INDIVIDUAL);
                    }
                    catch
                    {
                        return string.Empty;
        ...
        public string STAFF_BAND
        {
            get
            {
                using (DBHelpDataContext data = new DBHelpDataContext())
                {
                    try
                    {
                        return Common.ParseString(data.M_STAFF_BAND_SALARies.FirstOrDefault(c => c.STAFF_ID == this.STAFF_ID).STAFF_BAND);
                    }
                    catch
                    {
                        return string.Empty;
